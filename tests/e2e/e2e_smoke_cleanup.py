...
class RealServerSmokeCleanupE2ETest(unittest.TestCase):
    def test_smoke_json_exercises_real_server_and_cleanup(self):
        ...
        self._assert_smoke_skipped_blocking_execution(payload, context)
    ...
    def _assert_smoke_capabilities(self, capabilities, context):
        ...
        for field in ("v2_prompt_support", "event_support"):
            self.assertIs(capabilities.get(field), True, context)
        self.assertIsInstance(capabilities.get("legacy_fallback_available"), bool, context)
        routes = capabilities.get("route_availability")
        self.assertIsInstance(routes, dict, context)
        for name in ("session", "v2_prompt", "events"):
        ...
        for name in ("legacy_run", "legacy_reply"):
            route = routes.get(name)
            self.assertIsInstance(route, dict, context)
            self.assertIsInstance(route.get("available"), bool, context)
            self.assertIsInstance(route.get("path"), str, context)
            self.assertNotEqual(route.get("path"), "", context)
    ...
    def _assert_smoke_skipped_blocking_execution(self, payload, context):
        ...
        self.assertEqual(
            (run_blocking.get("fallback") or {}).get("available"),
            payload["capabilities"]["legacy_fallback_available"],
            context,
        )
    ...
    def _assert_session_readable(self, server_url, session_id, directory):
        ...
        self.assertEqual(location.get("directory") or session.get("directory") or session.get("cwd"), directory, context)
