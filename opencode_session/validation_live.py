...
def run_live_validate(args, client, *, print_error, unavailable_exit=EX_UNAVAILABLE, unsupported_exit=EX_UNSUPPORTED):
    ...
    def validate(harness):
        ...
        require_live_validate_capabilities(capabilities, unsupported_exit=unsupported_exit)
        result["checks"]["wait"] = live_wait_record(capabilities)
        ...
        steer.update(live_steer_execution_observation(client, steer, capabilities))
        ...
        try:
            run_blocking = execute_blocking_prompt(client, run_session_id, LIVE_VALIDATE_PROMPT, capabilities)
