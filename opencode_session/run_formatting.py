...
from opencode_session.worker_state import is_worker_mapping, normalize_worker
...
def _worker_status_counts(workers):
    ...
    for worker_id, worker in workers.items():
        status = normalize_worker(worker, worker_id).get("status") if is_worker_mapping(worker) else None
        if status in counts:
            counts[status] += 1
    return counts
