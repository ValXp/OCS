...
class SessionEventWatcher:
    ...
    def iter_events(self, *, deadline=None, on_open=None, stop_event=None):
        stream_kwargs = {"on_open": on_open, "deadline": deadline}
        if stop_event is not None:
            stream_kwargs["stop_event"] = stop_event
...
class BackgroundSessionEventWatcher:
    ...
    def close(self, timeout=1.0):
        self.closed.set()
        if self.thread.ident is None or threading.current_thread() is self.thread:
    ...
    def _run(self):
        try:
            for event in self.watcher.iter_events(on_open=self.opened.set, stop_event=self.closed):
